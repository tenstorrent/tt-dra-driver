...
// LoggingConfig wraps the upstream component-base logging configuration so it
// can be exposed through urfave/cli flags.
type LoggingConfig struct {
	featureGate featuregate.MutableVersionedFeatureGate
	config      *logsapi.LoggingConfiguration
}

// NewLoggingConfig returns a LoggingConfig with contextual logging enabled by
// default. Callers must invoke Apply after parsing flags.
...
func (l *LoggingConfig) Flags() []cli.Flag {
	...
	// Adding the feature gates flag to fs means that it is going to be added
	// with "logging" as category. In practice, the logging code is the only
	// code which uses the flag, therefore that seems like a good place to
	// report it.
	fs.AddFlag(&pflag.Flag{
		...
		Value: l.featureGate.(pflag.Value), //nolint:forcetypeassert // l.featureGate is a *featuregate.featureGate, which implements pflag.Value.
	})
